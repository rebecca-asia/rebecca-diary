...
Phase 0（静的日記）完了 → Phase 1（Room Status + Health）完了 → Phase 1.5（ドメインレイヤー確立）完了。
...
│   ├── data/           # Phase 1: collected JSON data (gitignored, generated by collectors)
...
│   ├── collect_nurture.py # 育成パラメータ計算（constants のみ参照）
│   └── collect_skills.py  # スキル収集（constants のみ参照）
├── screenshot.png
